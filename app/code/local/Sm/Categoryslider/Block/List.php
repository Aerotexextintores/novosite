...
class Sm_Categoryslider_Block_List extends Mage_Catalog_Block_Product_Abstract
{
	...
	public function _getCfg($attr = null)
	{
		...
		$def_cfgs = Mage::getConfig()
									...
									->getNode('default/categoryslider_cfg')->asArray(); 
		...
		$_configs = (array)Mage::getStoreConfig("categoryslider_cfg");
		if(empty($_configs)) return;
		$cfgs = array();
		
		foreach($groups as $group){
			$_cfgs = Mage::getStoreConfig('categoryslider_cfg/'. $group.'');
	...
	protected function _toHtml(){
		if(!$this->_getConfig('active',1)) return;
		//$template_file = "sm/categoryslider/default.phtml";
